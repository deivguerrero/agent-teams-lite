...
1. Scan user skills: glob `*/SKILL.md` across ALL known skill directories. **User-level**: `~/.claude/skills/`, `~/.config/opencode/skills/`, `~/.gemini/skills/`, `~/.gemini/antigravity/skills/`, `~/.codex/skills/`, parent of this skill file. **Project-level**: `.claude/skills/`, `.codex/skills/`, `.gemini/skills/`, `.agent/skills/`, `.vscode/skills/`, `skills/`. Skip `sdd-*`, `_shared`, `skill-registry`. Deduplicate by name (project-level wins). Read frontmatter triggers.
